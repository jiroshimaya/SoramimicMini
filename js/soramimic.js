...
class Soramimic {
	constructor(){
		this.DEFAULT_PARAMETER_VALUES_ = SoramimicPublic.getDefaultParam();
		
		this.KANA2PHONON_ = SoramimicPublic.getKanaToPhonon();
		this.VOWELS_ = SoramimicPublic.getVowels();
		this.SMALL_VOWELS_ = SoramimicPublic.getSmallVowels();
		this.LARGE_VOWELS_ = SoramimicPublic.getLargeVowels();
		this.SMALL2LARGE_ = SoramimicPublic.getSmallToLarge();

		this.KANA2VOWEL_ = SoramimicPublic.getKanaToVowel();
		this.KANA2CONSONANT_ = SoramimicPublic.getKanaToConsonant();
		this.KANA_UNITS_ = SoramimicPublic.getKanaUnits();
		this.KANA_UNITS_LIST_ = Object.keys(this.KANA_UNITS_);

		this.VOWEL_SIMILARITY_ = SoramimicPublic.getVowelSimilarity();
		this.CONSONANT_SIMILARITY_ = SoramimicPublic.getConsonantSimilarity();
		this.ENGLISH2KANA_ = SoramimicPublic.getEnglishToKana();//key値のアポストロフィはAPOSTROPHEに変換されている
		
		console.time("gksb");
		//this.KANA_SIMILARITY_BASE_ = SoramimicPublic.getKanaSimilarityBase();
		this.KANA_SIMILARITY_ = SoramimicPublic.getKanaSimilarity();//ひらがなの置換コストの微調整後の値
	...
	soramimi_dp(text, wordlist, para = {}){
		...
		const param = SoramimicPublic.getFormatedParam(para);
	...
	set kanaSimilarity(param){
		...
		this.KANA_SIMILARITY_ = SoramimicPublic.getKanaSimilarity(param);
	}
